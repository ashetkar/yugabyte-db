...
(YUGABYTEDB_CONNECTION_STRING) when TARGET_DB_CONNECTION_STRING is unset --
...
class RegistrationPoller:
    ...
    def _find_candidates(
        ...
        if not registration.destination_priority_column:
            ...
            query = sql.SQL(
                ...
                "   ON p.registration_id = %(reg_id)s AND p.row_pk = d.{pk}::text"
    ...
    def _claim_candidates(
        ...
        query = sql.SQL(
            ...
            "    (registration_id, row_pk, status, claimed_by, claimed_at, attempts)"
            " SELECT %(reg_id)s, x.row_pk, 'IN_PROGRESS', %(worker_id)s, NOW(), 0"
            " FROM unnest(%(candidate_pks)s::text[]) AS x(row_pk)"
            " ON CONFLICT (registration_id, row_pk) DO UPDATE"
    ...
    def finalize_success(self, claimed_row: ClaimedRow, embedding: List[float]) -> None:
        ...
        try:
            ...
            cursor.execute(
                ...
                " WHERE registration_id = %s AND row_pk = %s",
            ...
            cursor.execute(
                ...
                SET attempts = attempts + 1,
                    ...
                    next_retry_at = CASE WHEN attempts + 1 >= %(max_attempts)s
                             ...
                             END
                WHERE registration_id = %(reg_id)s AND row_pk = %(row_pk)s
                RETURNING status;
                """,
                {
                    "error": error,
