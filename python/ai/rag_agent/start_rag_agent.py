...
AUTO_COLUMN_EMBEDDING_POLL_IDLE_SLEEP = int(
    os.getenv("AUTO_COLUMN_EMBEDDING_POLL_IDLE_SLEEP_SECONDS", "1")
)
AUTO_COLUMN_EMBEDDING_POLL_ERROR_BACKOFF = int(
    os.getenv("AUTO_COLUMN_EMBEDDING_POLL_ERROR_BACKOFF_SECONDS", "60")
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if worker_type == WORKER_TYPE_AUTO_COLUMN_EMBEDDING:
            ...
            # runs against SOURCE_DB_CONNECTION_STRING (see
            # rag_pipeline.source_readers). TARGET_DB_CONNECTION_STRING
            ...
            # no sensible default.
            source_db_connection_string = os.getenv("SOURCE_DB_CONNECTION_STRING")
            if not source_db_connection_string:
                raise ValueError(
                    "SOURCE_DB_CONNECTION_STRING environment variable is "
            ...
            target_db_connection_string = (
                os.getenv("TARGET_DB_CONNECTION_STRING") or db_connection_string
            )
