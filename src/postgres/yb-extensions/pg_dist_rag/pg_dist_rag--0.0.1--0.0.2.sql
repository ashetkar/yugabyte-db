...
CREATE TABLE dist_rag.column_embedding_progress (
  ...
  row_pk           TEXT NOT NULL,   -- destination row's PK value, as text -- generic across UUID/int/etc.
  ...
  last_error       TEXT,
  updated_at       TIMESTAMP NOT NULL DEFAULT NOW(),
  PRIMARY KEY (registration_id, row_pk)
);
...
SELECT
    ...
    COUNT(p.row_pk) FILTER (WHERE p.status = 'QUEUED')      AS pending_count,
    COUNT(p.row_pk) FILTER (WHERE p.status = 'IN_PROGRESS') AS in_progress_count,
    COUNT(p.row_pk) FILTER (WHERE p.status = 'FAILED')      AS failed_count,
...
GRANT ALL ON dist_rag.column_embedding_registrations TO PUBLIC;
GRANT ALL ON dist_rag.column_embedding_progress TO PUBLIC;
GRANT ALL ON dist_rag.column_embedding_registration_stats TO PUBLIC;
