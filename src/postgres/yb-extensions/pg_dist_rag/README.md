...
| `dist_rag.column_embedding_progress` | In-flight/failed lease-retry state for column embedding, keyed by `(registration_id, row_pk)` -- never on the destination table |
